...
use pqhybridsign_core::{component::ClassicalScheme, suite::DeltaSuite};
...
/// Length in bytes of an H2 public key.
pub const HYBRID_PK_LEN: usize = fndsa512::HYBRID_PK_LEN;
/// Length in bytes of an H2 secret key.
pub const HYBRID_SK_LEN: usize = fndsa512::HYBRID_SK_LEN;
/// Maximum length in bytes of an H2 signature.
...
pub type PublicKey = fndsa512::PublicKey<Ed25519FnDsa512>;
/// H2 secret key, zeroized on drop.
pub type SecretKey = fndsa512::SecretKey<Ed25519FnDsa512>;
/// Fixed-size, zero-padded H2 signature.
pub type Signature = fndsa512::Signature<Ed25519FnDsa512>;

/// Zero-sized type implementing [`HybridSignatureScheme`] for H2.
...
impl HybridSignatureScheme for Ed25519FnDsa512 {
...
mod tests {
	...
	fn roundtrip_and_tamper_rejection() {
		let (secret, public) = Ed25519FnDsa512::generate(&mut OsRng);
		let signature = Ed25519FnDsa512::sign(&secret, b"hello quip", b"h2", &mut OsRng);
		assert!(Ed25519FnDsa512::verify(&public, b"hello quip", b"h2", &signature));

		let mut tampered = signature.to_bytes();
		tampered[100] ^= 1;
		let tampered = Signature::from_bytes(&tampered).expect("fixed-size signature");
		assert!(!Ed25519FnDsa512::verify(&public, b"hello quip", b"h2", &tampered));
	}

	#[test]
	fn deterministic_signing_is_reproducible() {
		let (secret, _) = Ed25519FnDsa512::from_seed_slice(&[7u8; 32]).expect("seed");
		let first = Ed25519FnDsa512::sign_deterministic(&secret, b"message", b"context", b"nonce");
		let second = Ed25519FnDsa512::sign_deterministic(&secret, b"message", b"context", b"nonce");
		assert_eq!(first.as_ref(), second.as_ref());
	}

	#[test]
	fn public_key_recovers_from_serialized_secret() {
		let (secret, public) = Ed25519FnDsa512::from_seed_slice(&[9u8; 32]).expect("seed");
		let reparsed = SecretKey::from_bytes(secret.to_bytes().as_ref()).expect("secret");
		assert_eq!(Ed25519FnDsa512::public(&reparsed).as_ref(), public.as_ref());
	}

	#[test]
	fn matches_pqhybridsign_h2_golden_vector() {
		let vector: Vector = serde_json::from_str(include_str!("../../tests/vectors/h2.json"))
			.expect("valid H2 vector");
		...
		let (secret, public) = Ed25519FnDsa512::from_seed_slice(&seed).expect("keygen");
		let signature = Ed25519FnDsa512::sign_deterministic(&secret, &msg, &ctx, &nonce);

		assert_eq!(hex::encode(public.as_ref()), vector.public_key_hex);
		assert_eq!(hex::encode(signature.wire_bytes()), vector.signature_hex);
		assert!(Ed25519FnDsa512::verify(&public, &msg, &ctx, &signature));
	}
}
